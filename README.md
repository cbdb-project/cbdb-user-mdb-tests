...
| 12 | 🟡 partial | First slice landed: `tests/test_index_year_xcheck.py` + `analysis/download_hf_sqlite.py` (fetches the weekly SQLite from <https://huggingface.co/datasets/cbdb/cbdb-sqlite/blob/main/latest.zip>). The test compares both DERIVED fields (`c_index_year`, `c_index_addr_id`) AND SOURCE fields (`c_birthyear`, `c_deathyear`) per `c_personid`. Latest full sweep on 657 246 common persons: **83 `c_index_year` + 492 `c_index_addr_id` + 12 `c_birthyear` + 12 `c_deathyear` disagreements**. Root cause classified — see findings.md Open Question #1: both pipelines run the same algorithm (cbdb-online-main-server's `IndexYearRebuildService.php` Phase A/B/C), the User MDB has no own derivation in VBA, the differences are mostly source-data sync gaps (12 persons with divergent birth/death years) propagating downstream into the derivation, plus tiebreaks in Phase C kinship-propagation loops. Source threshold tightened to 0.1 % so meaningful drift surfaces fast |
...
- 🟡 比對 User MDB 的 `index year` / `index address` 算法與 [`cbdb-online-main-server`](https://github.com/cbdb-project/cbdb-online-main-server) — roadmap 第 12 項：基础设施 + 首切发现都上线。算法源头是 cbdb-online-main-server 的 `IndexYearRebuildService.php`（User MDB 自己不算，吃 CBDB 主库 pipeline 的预算结果）。最新跑：83 個 `c_index_year` 差异 + 492 個 `c_index_addr_id` 差异 + **12 個 `c_birthyear` 差异**（source-data drift —— 比 derivation tiebreak 更值得追）。详细分析见 findings.md Open Question #1
