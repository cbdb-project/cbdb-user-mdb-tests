...
| LookAtKinship          | ✅ 1 fixture | ✅ CmdGIS (struct, requery shim) + ✅ CmdUCINet (.vna 3-section, family first coverage cell, `tests/test_vba_cmducinet_kinship.py`) | ✅ Store + ✅ Recall | ✅ CmdImport | — (no save button) | 949 rows (Zhao Tingmei) |
| LookAtAssociationPairs | ✅ tiny fixture (`test_vba_matrix_hard_forms.py`, 4×5) | ✅ CmdPajek (struct) + ✅ CmdGephi (struct) via 1×3 known-edged person pair (`test_vba_pajek_gephi_cross_form.py`) | ✅ Recall | ✅ CmdImportList | — (no save button) | `Link1stOrder` ASSOC_DATA self-join too slow at full scale; tiny fixture covers CmdQuery happy-path; CmdPajek/CmdGephi unblocked by AssociationPairs SetFocus driver patch |
...
| 8 | 🟡 partial | First slice landed: `tests/test_vba_cmdgis_other_forms.py` extends real-`CmdGIS` coverage from LookAtEntry (`test_vba_export.py`, byte-diff) to Status / Texts / Associations / Office / Place / Kinship (6 added). Uses **structural** assertions rather than byte-diff. **Subsequent slices** (added 2026-05-02): `test_vba_cmdguess_cross_form.py` (Kinship + Office), `test_vba_pajek_gephi_cross_form.py` (7 passing — 5 original + AssociationPairs CmdPajek + CmdGephi via 1×3 known-edged pair, unblocked by the AssociationPairs SetFocus driver patch; 2 Status skipped), `test_vba_cmdgispeople_office.py`, `test_vba_cmdneo4j_cross_form.py` (4 passing — Entry / Texts / Office / Kinship; 3 skipped — Status / Place / Associations; **LookAtEntry promoted from skip 2026-05-04** with the c_entry_code=101 matrix fixture from the Issue #9 reverification probe, plus a structural assertion that the file set is exactly {People, PeopleEntry, Places, PeoplePlaces, PersonPlaceCodes, EntryCodes, AssocCodes} AND no InstitutionCodes file — pins the Issue #9 LATENT-gate.  Surfaced **3 new bugs** in the CmdNeo4j family: Bugs #7 / #8 / #9). Driver gained: `patch_filedialog` v8 with directory mode (counter-suffixed unique file per call) for multi-dialog subs like CmdNeo4j. **Still open:** CmdKML across all forms; CmdUCINet first coverage cell landed on LookAtKinship (`tests/test_vba_cmducinet_kinship.py`, .vna 3-section family shape established by probe `investigate/cmducinet-family-shape`); Associations CmdUCINet still blocked (mid-write VBA error 5 per probe), Place CmdUCINet still gap (uses ADO Stream — needs separate per-form manifest), other forms unchanged |
...
已找到的 bug 在 [`reports/CBDB_Issues_Report_ZH-Hant.md`](./reports/CBDB_Issues_Report_ZH-Hant.md)（共 20 個 issue，由 `reports/generate_report.py` 自動生成 4 份輸出）。
...
- 🟡 其他匯出按鈕大多已部分涵蓋：CmdGIS（6 個 form, `test_vba_cmdgis_other_forms.py`）、CmdGUESS（Kinship+Office, `test_vba_cmdguess_cross_form.py`）、CmdPajek/Gephi（`test_vba_pajek_gephi_cross_form.py`，7 過 — 原 5 過 + AssociationPairs 的 CmdPajek/CmdGephi 透過 1×3 known-edged pair 解開（依賴 SetFocus driver patch）/ 2 Status skip）、CmdGISPeople（Office）、CmdNeo4j（4 過 / 3 skip — Entry/Texts/Office/Kinship 通過；Status/Place/Associations 跳過。Entry 於 2026-05-04 從 skip 提升為 covered，固定 Issue #9 LATENT-gate。順手挖出 Bug #7/#8/#9）。CmdKML 仍未測；CmdUCINet 的 family first coverage cell 已落地於 LookAtKinship（`tests/test_vba_cmducinet_kinship.py`，.vna 三段結構由 probe `investigate/cmducinet-family-shape` 確立），Associations CmdUCINet 仍因匯出途中 VBA error 5 受阻，Place CmdUCINet 仍 gap（使用 ADO Stream，需要單獨 per-form 處理）— roadmap 第 8 項
