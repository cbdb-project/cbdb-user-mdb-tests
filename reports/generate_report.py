...
ISSUES = [
    ...
    },
    {
        ...
        "summary_en": (
            ...
            "**Sibling-form risk (NOT yet probed in this "
            "filing):** `Form_LookAtKinship.CmdUCINet_Click` "
            "uses the same `CreateTextFile(tFileName, True)` "
            "pattern at line ~2510.  The current Kinship × "
            "CmdUCINet coverage test "
            "(`tests/test_vba_cmducinet_kinship.py`) passes "
            "only because person 3211's network happens to "
            "have no Han-character c_name values; a "
            "different Kinship fixture with such names would "
            "likely surface the same crash there.  "
            "`Form_LookAtPlace.CmdUCINet_Click` uses ADO "
            "Stream rather than FSO so its encoding "
            "behaviour is potentially different and would "
            "need its own probe.  This issue canonicalizes "
            "ONLY the Associations finding because that's "
            "all the probe directly verified."
        ),
        "summary_zh": (
            ...
            "取。\n\n"
            "**Sibling-form 风险（本 issue 尚未直接 probe "
            "确认）：** `Form_LookAtKinship.CmdUCINet_Click` "
            "在约 line 2510 用同样的 `CreateTextFile"
            "(tFileName, True)` 模式。当前 Kinship × CmdUCINet "
            "覆盖测试（`tests/test_vba_cmducinet_kinship."
            "py`）能通过，仅是因为 person 3211 的网络刚好没"
            "有带汉字的 c_name；换个含汉字 c_name 的 Kinship "
            "fixture 大概率会在同一处崩溃。`Form_LookAtPlace."
            "CmdUCINet_Click` 用的是 ADO Stream（不是 FSO），"
            "编码行为可能不同，需要单独 probe 验证。本 issue "
            "只 canonicalize probe 已直接证实的 Associations "
            "层面。"
        ),
        "steps_en": [
        ...
        "fix_en": (
            ...
            "tFileName, True, True)\n"
            "```\n\n"
            "This makes `tVNA.WriteLine` write all "
            "characters as UTF-16LE; cp1252-only chars no "
            "longer crash, and downstream consumers "
            "(UCINET, Visone) accept UTF-16 `.vna`.\n\n"
            ...
            "addition) likely needs to be applied to "
            "`Form_LookAtKinship.CmdUCINet_Click` (line "
            "~2510) — see the Sibling-form risk paragraph "
            "in the description.  That sibling fix is "
            "out-of-scope for this issue's canonical "
            "verification but worth coordinating in the "
            "same upstream patch."
        ),
        "fix_zh": (
            ...
            "tFileName, True, True)\n"
            "```\n\n"
            "这样 `tVNA.WriteLine` 就用 UTF-16LE 写入所有"
            "字元，cp1252 无法表示的字元不再造成崩溃，下游"
            "的 UCINET / Visone 也都接受 UTF-16 的 `.vna` "
            "档。\n\n"
            ...
            "2510）大概率需要套用同样的一行修改 —— 详见描"
            "述里的 Sibling-form 风险段。本 issue 的 canonical "
            "验证范围不含 Kinship；上游修补时建议一起处理。"
        ),
        "screenshots": [],
        "severity_en": (
            "P1 — Visible crash on a normal user click.  "
