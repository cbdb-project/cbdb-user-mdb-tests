...
def _add_index_drift_appendix(doc, is_en: bool, Z) -> None:
    ...
    intro = (
        ...
        "the User MDB's same two fields are produced by the Access "
        "`frmBaseMaintenance` VBA maintenance buttons.  PHP is "
        ...
        "`frmBaseMaintenance` 維護按鈕底下的 VBA 算出來的。PHP "
...
def _build_md(lang: str, out_path: Path) -> None:
    ...
    if DRIFT_JSON.exists():
        ...
        intro_drift = (
            ...
            "<https://github.com/cbdb-project/cbdb-online-main-server>"
            "); the User MDB's same two fields are produced by the "
            "Access `frmBaseMaintenance` VBA maintenance buttons.  "
            "PHP is intended to mirror the "
            ...
            "欄位則是由 Access `frmBaseMaintenance` 維護按鈕底下的 "
            "VBA 算出來的。PHP **意圖**鏡像 VBA，"
