...
ISSUES = [
    ...
    },
    {
        "id": 23,
        "tier": "P1_visible_crash",
        "form": "Form_LookAtAssociations.CmdNeo4j_Click",
        "title_en": "LookAtAssociations.CmdNeo4j INSERT references non-existent target column ZZ_SCRATCH_PEOPLE.c_index_addr_type_code (likely intended c_addr_type)",
        "title_zh": "LookAtAssociations.CmdNeo4j 的 INSERT 引用了 ZZ_SCRATCH_PEOPLE 上不存在的目标列 c_index_addr_type_code（疑似本意为 c_addr_type）",
        "summary_en": (
            "`Form_LookAtAssociations.CmdNeo4j_Click` builds the "
            "`ZZ_SCRATCH_PEOPLE` working table via an "
            "`INSERT INTO ZZ_SCRATCH_PEOPLE ( c_person_id, c_name, "
            "c_name_chn, c_index_year, c_index_year_type_code, "
            "c_dy, c_addr_id, c_index_addr_type_code, c_female ) "
            "SELECT DISTINCT … BIOG_MAIN.c_index_addr_type_code, "
            "BIOG_MAIN.c_female …`. The INSERT target list "
            "references **c_index_addr_type_code** as a target "
            "column on `ZZ_SCRATCH_PEOPLE`, but `ZZ_SCRATCH_PEOPLE` "
            "has no such column on the current dump (22 columns "
            "total; canonical column names in "
            "`analysis/dump/tables.json`).  JET reports this as "
            "**3061 'The INSERT INTO statement contains the "
            "following unknown field name: c_index_addr_type_code'** "
            "and the entire Neo4j export aborts mid-body, BEFORE "
            "any disk file is written — even though `dlgSaveAs.Show` "
            "did fire and the chain entered the People-block True "
            "branch.\n\n"
            "Static schema cross-check: the source side `BIOG_MAIN` "
            "DOES have `c_index_addr_type_code` (55 columns total; "
            "this column is one of `tests/test_schema.py`'s "
            "REQUIRED_COLUMNS so a passing schema test confirms it "
            "independently). So the unknown field is on the "
            "**target** table, not the source.\n\n"
            "Strong static inference about author intent: the "
            "follow-up `UPDATE` (next statement after the failing "
            "INSERT) LEFT JOINs on "
            "`ZZ_SCRATCH_PEOPLE.c_addr_type = "
            "BIOG_ADDR_CODES.c_addr_type` and SETs descriptive "
            "address columns from `[BIOG_ADDR_CODES]`. Target table "
            "DOES have `c_addr_type`, but the failing INSERT never "
            "populates `c_addr_type` — and `c_addr_type` is the "
            "natural rename target for the source's "
            "`BIOG_MAIN.c_index_addr_type_code` (the same INSERT "
            "already does the analogous rename in the previous "
            "column position: `BIOG_MAIN.c_index_addr_id ↦ "
            "ZZ_SCRATCH_PEOPLE.c_addr_id`). The author appears to "
            "have copied the source column name verbatim into the "
            "INSERT target list when they meant to rename it to "
            "`c_addr_type`. Same per-form column-name typo class as "
            "Bugs #4 / #5 / #6.\n\n"
            "**Distinct from Issue #22 (LookAtAssociations × "
            "CmdUCINet).** That issue is the FSO `CreateTextFile` "
            "missing-Unicode-arg crash on CJK Han characters via "
            "ANSI cp1252 encoder; this issue is a SQL JET 3061 "
            "column-not-found in `CmdNeo4j_Click` that fires before "
            "any export-encoder runs.  Different sub, different "
            "VBA error family, different fix path.\n\n"
            "**Distinct from AssociationPairs × CmdNeo4j blocking-"
            "MsgBox layer** (canonical evidence: PR #109 driver "
            "patch + PR #110 coverage on main).  AssocPairs's "
            "CmdNeo4j writes ≥1 files first and is then blocked by "
            "a UI debug-MsgBox layer (now driver-suppressed). "
            "Associations writes 0 files and is blocked by the SQL "
            "schema mismatch above.  Different chain depths, "
            "different VBA error families, no shared fix path — "
            "they should NOT be filed as the same issue."
        ),
        "summary_zh": (
            "`Form_LookAtAssociations.CmdNeo4j_Click` 通过 "
            "`INSERT INTO ZZ_SCRATCH_PEOPLE ( c_person_id, c_name, "
            "c_name_chn, c_index_year, c_index_year_type_code, "
            "c_dy, c_addr_id, c_index_addr_type_code, c_female ) "
            "SELECT DISTINCT … BIOG_MAIN.c_index_addr_type_code, "
            "BIOG_MAIN.c_female …` 来填充 `ZZ_SCRATCH_PEOPLE` 工作"
            "表。INSERT 目标列里把 **c_index_addr_type_code** 当作 "
            "`ZZ_SCRATCH_PEOPLE` 上的列引用，但当前 dump 的 "
            "`ZZ_SCRATCH_PEOPLE` 共 22 列，并没有这一列（canonical "
            "列名见 `analysis/dump/tables.json`）。JET 报 "
            "**3061「INSERT INTO 语句包含未知的字段名 "
            "c_index_addr_type_code」**，整个 Neo4j 汇出在 body 中"
            "途中断 —— 在任何磁盘文件被写出之前 —— 即便 "
            "`dlgSaveAs.Show` 已经弹出且链条已进入 People-block "
            "True 分支。\n\n"
            "静态 schema 互相印证：source 端 `BIOG_MAIN` **有** "
            "`c_index_addr_type_code`（共 55 列；该列在 "
            "`tests/test_schema.py` 的 REQUIRED_COLUMNS 中，schema "
            "测试通过即独立证实）。所以缺的是 **target** 表，不是 "
            "source。\n\n"
            "关于作者意图的强静态推断：失败 INSERT 之后紧邻的 "
            "`UPDATE` 是 LEFT JOIN "
            "`ZZ_SCRATCH_PEOPLE.c_addr_type = "
            "BIOG_ADDR_CODES.c_addr_type` 并 SET 一组地址描述列。"
            "target 表 **有** `c_addr_type`，但失败的 INSERT 从未"
            "写入 `c_addr_type` —— 而 `c_addr_type` 正是 source "
            "`BIOG_MAIN.c_index_addr_type_code` 的自然 rename 目标"
            "（同一 INSERT 在前一列已经做过一模一样的 rename："
            "`BIOG_MAIN.c_index_addr_id ↦ "
            "ZZ_SCRATCH_PEOPLE.c_addr_id`）。看起来作者把 source "
            "列名直接复制到 INSERT 目标列表里，本意是 rename 成 "
            "`c_addr_type`。与 Bug #4 / #5 / #6 同形（per-form "
            "column-name typo class）。\n\n"
            "**与 Issue #22（LookAtAssociations × CmdUCINet）"
            "无关。** Issue #22 是 FSO `CreateTextFile` 缺 Unicode "
            "参数导致 CJK 汉字走 ANSI cp1252 时崩溃；本 issue 是 "
            "`CmdNeo4j_Click` 里 SQL JET 3061 列不存在，在任何 "
            "export-encoder 运行之前就触发。不同 sub、不同 VBA "
            "错误族、不同修法。\n\n"
            "**与 AssociationPairs × CmdNeo4j 的 blocking-MsgBox "
            "层不同**（canonical 证据：main 上 PR #109 的 driver "
            "patch + PR #110 的 coverage）。AssocPairs 的 CmdNeo4j "
            "会先写出 ≥1 个文件再被 UI debug-MsgBox 层挡住（现已"
            "由 driver suppress）。Associations 的 CmdNeo4j 写 0 "
            "个文件就被上面的 SQL schema 不匹配挡住。两者链条深度"
            "不同、VBA 错误族不同、修法路径无重叠 —— 不应合并为"
            "同一 issue。"
        ),
        "steps_en": [
            "Open **LookAtAssociations**.",
            "Pick a substantive **association code** (any single "
            "code with ≥ a few hundred associated persons works; "
            "the matrix `_make_assoc_fixtures` first fixture, "
            "`assoc_<top_code>_unfiltered`, gives ZZ_SCRATCH_ASSOC ≈ "
            "11,867 rows + ZZ_SCRATCH_P_ASSOC ≈ 8,087 rows on the "
            "current dump — same fixture used by the existing "
            "Associations × CmdGIS / CmdPajek / CmdGephi tests).",
            "Leave **FrameFilterYears** at its default (no year "
            "filter).  Do NOT tick any checkbox that gates an "
            "unrelated query branch.",
            "Click **Run Query** — CmdQuery completes cleanly; "
            "the underlying scratch tables are populated.",
            "Click **Neo4j** (the export button).",
            "A **Run-time error 3061 — The INSERT INTO statement "
            "contains the following unknown field name: "
            "c_index_addr_type_code** popup appears (or in the "
            "headless / driver-instrumented run, the equivalent "
            "ZZ_TEST_DEBUG marker `LookAtAssociations:ERR ...` is "
            "written).  The Neo4j export produces **0 CSV files** — "
            "no `People_*.csv`, no `Places_*.csv`, nothing.\n\n"
            "Verified end-to-end via the probe at "
            "`analysis/probe_associations_cmdneo4j.py` (PR #112, "
            "merged commit `1145219`); root cause confirmed via "
            "the static investigation at "
            "`analysis/investigate_associations_cmdneo4j_"
            "c_index_addr_type_code.{py,md}` (PR #114, merged "
            "commit `68cfa9b`).",
        ],
        "steps_zh": [
            "打开 **LookAtAssociations**。",
            "选一个有实质数据的 **association code**（任一关联人数"
            "上百的 code 都可以；matrix `_make_assoc_fixtures` 第一"
            "条 fixture `assoc_<top_code>_unfiltered` 在当前 dump "
            "上能让 ZZ_SCRATCH_ASSOC 约 11,867 行、"
            "ZZ_SCRATCH_P_ASSOC 约 8,087 行 —— 同 Associations × "
            "CmdGIS / CmdPajek / CmdGephi 测试用的是同一条 "
            "fixture）。",
            "**FrameFilterYears** 保持默认（不筛年）。不要勾任何"
            "与本次查询无关的子分支。",
            "点 **Run Query** —— CmdQuery 顺利完成，底层 scratch "
            "表被填充。",
            "点 **Neo4j**（汇出按钮）。",
            "弹出 **执行时错误 3061 —— INSERT INTO 语句包含未知的"
            "字段名 c_index_addr_type_code** 对话框（或 headless "
            "／driver-instrumented 跑法下，对应的 "
            "`LookAtAssociations:ERR ...` ZZ_TEST_DEBUG marker 被"
            "写入）。Neo4j 汇出产出 **0 份 CSV** —— 没有 "
            "`People_*.csv`、没有 `Places_*.csv`，什么都没有。\n\n"
            "已通过 probe `analysis/probe_associations_cmdneo4j.py`"
            " 端到端验证（PR #112，已 merge `1145219`）；根因经"
            "静态调查 `analysis/investigate_associations_cmdneo4j_"
            "c_index_addr_type_code.{py,md}` 确认（PR #114，已 "
            "merge `68cfa9b`）。",
        ],
        "fix_en": (
            "**Recommended upstream CBDB fix:** rename the INSERT "
            "target column from `c_index_addr_type_code` to "
            "`c_addr_type`. The follow-up UPDATE later joins on "
            "`ZZ_SCRATCH_PEOPLE.c_addr_type = "
            "BIOG_ADDR_CODES.c_addr_type`, so this rename has the "
            "side benefit of populating the join key the UPDATE "
            "already requires (today the join key is never "
            "populated, so even a hypothetical 'just remove the "
            "extra column' patch would leave the UPDATE silently "
            "joining on NULL).\n\n"
            "Suggested shape (rename one identifier; SELECT side "
            "unchanged because the source column name is correct):\n\n"
            "```vb\n"
            ...
            "NOT part of this issue): mirror the `_PER_FORM_CMDGIS_"
            "PATCHES` Issue #4 (`GISFrame → CodeFrame`) and Issue "
            "#5 (`ChkIDs → False`) workarounds with a per-form "
            "rewrite mapping the literal `c_index_addr_type_code` "
            "→ `c_addr_type` inside `Form_LookAtAssociations`'s "
            "CmdNeo4j_Click only.  This would unblock the test "
            "suite without requiring a CBDB-side fix."
        ),
        "fix_zh": (
            "**推荐的上游 CBDB 修复：** 把 INSERT 目标列名从 "
            "`c_index_addr_type_code` 改为 `c_addr_type`。紧邻的 "
            "UPDATE 本来就在 `ZZ_SCRATCH_PEOPLE.c_addr_type = "
            "BIOG_ADDR_CODES.c_addr_type` 上 LEFT JOIN，这个 rename "
            "顺带还填上 UPDATE 早就需要的 join key（今天该 join "
            "key 从未被填充，所以即使只是「把那一列删掉」的修法也"
            "会让 UPDATE 静默地在 NULL 上 join）。\n\n"
            "建议写法（只改一个 identifier；SELECT 端不变，因为 "
            "source 列名是对的）：\n\n"
            "```vb\n"
            "tQueryStr = \"INSERT INTO ZZ_SCRATCH_PEOPLE ( \" & _\n"
            "    \"c_person_id, c_name, c_name_chn, c_index_year, \" & _\n"
            ...
            "    \"FROM ZZ_SCRATCH_P_TEXT INNER JOIN BIOG_MAIN ON ...\"\n"
            "```\n\n"
            "**Driver-side workaround 选项**（另一份 brief；本 "
            "issue 不包含）：仿照 `_PER_FORM_CMDGIS_PATCHES` 里 "
            "Issue #4（`GISFrame → CodeFrame`）和 Issue #5"
            "（`ChkIDs → False`）的写法，在仅限于 "
            "`Form_LookAtAssociations` 的 CmdNeo4j_Click 内做一个"
            "把字面量 `c_index_addr_type_code` 改写为 "
            "`c_addr_type` 的 per-form rewrite。这能让测试套不依赖"
            "上游修复就跑通。"
        ),
        "screenshots": [],
        "severity_en": (
            "P1 — Visible crash on a normal user click "
            "(any LookAtAssociations CmdNeo4j export with a "
            "non-empty associations result; the JET 3061 fires "
            "deterministically on the matrix Associations fixture "
            "per PR #112's runtime probe and the static schema "
            "evidence per PR #114)"
        ),
        "severity_zh": (
            "P1 — 正常用户点击下的可见报错（只要 LookAtAssociations "
            "查询有非空结果，CmdNeo4j 汇出就会触发；JET 3061 在 "
            "matrix Associations fixture 上稳定复现，runtime 证据"
            "在 PR #112 的 probe，静态 schema 证据在 PR #114 的"
            "调查）"
        ),
    },
    {
        ...
        "tier": "P1_visible_crash",
        "form": "Form_LookAtPlace.CmdNeo4j_Click",
        "title_en": "LookAtPlace.CmdNeo4j tRstPeople SELECT projection lacks c_dynasty / c_dynasty_chn / c_female; downstream loop reads them and crashes JET 3265 'Item not found in this collection'",
        "title_zh": "LookAtPlace.CmdNeo4j 中 tRstPeople 的 SELECT 投影缺少 c_dynasty / c_dynasty_chn / c_female，下游迴圈仍然讀取這些欄位並崩潰報「Item not found in this collection」（JET 3265）",
        "summary_en": (
            "`Form_LookAtPlace.CmdNeo4j_Click` opens a recordset "
            "into `tRstPeople` via "
            "`Set tRstPeople = CurrentDb.OpenRecordset(tQueryStr, "
            "dbOpenDynaset)` (line 651).  The bound SQL (lines "
            "643-647) projects only **four** columns from "
            "`ZZ_SCRATCH_P_TEXT`:\n\n"
            "```\n"
            "SELECT DISTINCT\n"
            ...
            "```\n\n"
            "The `INNER JOIN` brings `DYNASTIES` and `BIOG_MAIN` "
            "into scope, but the SELECT clause does not project "
            "any of their columns.  DAO's `Recordset.Fields` "
            "collection contains only the SELECT-projected "
            "columns; the JOIN is for filtering / row-shaping, "
            "NOT for field access.\n\n"
            "Despite this, the loop body (lines 689-783) reads "
            "**three columns from the JOINed tables that are "
            "NOT in the SELECT projection**:\n\n"
            "  - `tRstPeople!c_dynasty`     (line 757; expected "
            "from `DYNASTIES`)\n"
            "  - `tRstPeople!c_dynasty_chn` (line 769; expected "
            "from `DYNASTIES`)\n"
            "  - `tRstPeople!c_female`      (line 777, also 783; "
            "expected from `BIOG_MAIN`)\n\n"
            "JET reports this as **3265 'Item not found in this "
            "collection.'** on the FIRST such read "
            "(`!c_dynasty` at line 757).  The error trap routes "
            "to `Exit_CmdNeo4j_Click` BEFORE the chain's "
            "`gStream.WriteText` flushes any disk file, so the "
            "user sees a popup AND the export produces "
            "**0 CSV files** — even though the line-545 "
            "`dlgSaveAs.Show` already fired and captured a "
            "filename.\n\n"
            "Static schema cross-check (verified against "
            "`analysis/dump/tables.json`): `DYNASTIES` DOES have "
            "`c_dynasty` and `c_dynasty_chn`; `BIOG_MAIN` DOES "
            "have `c_female` (also in "
            "`tests/test_schema.py::REQUIRED_COLUMNS`).  So this "
            "is NOT source-side column rename / removal — the "
            "columns exist on the source tables, the SELECT just "
            "doesn't project them.  Pure recordset projection "
            "mismatch.\n\n"
            "**Distinct from Issue #23** (LookAtAssociations × "
            "CmdNeo4j target-column mismatch).  Issue #23 is JET "
            "3061 (SQL parser): an INSERT statement's target "
            "column list references a non-existent target table "
            "column.  This is JET 3265 (DAO field-collection "
            "lookup): a `Recordset!field` reference at the VBA "
            "layer can't find the field in the runtime field "
            "collection.  Same SURFACE root cause class (a "
            "missing column reference) but **different trigger "
            "surface** (DAO field lookup vs SQL parser) and "
            "**different fix surface** (the SELECT projection "
            "vs the INSERT target list).  Should not be merged "
            "into Issue #23.\n\n"
            "**Distinct from Issue #21** (LookAtGroupData × "
            "CmdNeo4j unguarded `.MoveFirst` on empty "
            "recordset).  Issue #21 is DAO 3021 'No current "
            "record' — a state-machine error on an empty but "
            "valid recordset.  This is a field-existence error "
            "on a non-empty recordset that simply doesn't "
            "expose the field.  Different DAO error code, "
            "different fix path."
        ),
        "summary_zh": (
            "`Form_LookAtPlace.CmdNeo4j_Click` 在第 651 行用 "
            "`Set tRstPeople = CurrentDb.OpenRecordset("
            "tQueryStr, dbOpenDynaset)` 開啟一個 recordset。"
            "綁定的 SQL（第 643-647 行）只從 "
            "`ZZ_SCRATCH_P_TEXT` 投影 **4** 個欄位：\n\n"
            "```\n"
            "SELECT DISTINCT\n"
            "    ZZ_SCRATCH_P_TEXT.c_person_id,\n"
            "    ZZ_SCRATCH_P_TEXT.c_name,\n"
            ...
            "ON ZZ_SCRATCH_P_TEXT.c_person_id = BIOG_MAIN.c_personid\n"
            "```\n\n"
            "`INNER JOIN` 把 `DYNASTIES` 和 `BIOG_MAIN` 帶入"
            "範圍，但 SELECT 子句沒投影這兩個表的任何欄位。"
            "DAO 的 `Recordset.Fields` 集合只包含 SELECT "
            "投影的欄位；JOIN 用於 filtering / row-shaping，"
            "**不**用於欄位存取。\n\n"
            "儘管如此，迴圈本體（第 689-783 行）仍然讀取 "
            "**3 個來自 JOINed 表但未被 SELECT 投影的欄位**：\n\n"
            "  - `tRstPeople!c_dynasty`     （第 757 行；預期來自 "
            "`DYNASTIES`）\n"
            "  - `tRstPeople!c_dynasty_chn` （第 769 行；預期來自 "
            "`DYNASTIES`）\n"
            "  - `tRstPeople!c_female`      （第 777、783 行；預期"
            "來自 `BIOG_MAIN`）\n\n"
            "JET 在第一次此類讀取時報 **3265「Item not found "
            "in this collection」**（即第 757 行 "
            "`!c_dynasty`）。錯誤處理跳到 "
            "`Exit_CmdNeo4j_Click`，**在 `gStream.WriteText` "
            "把任何資料寫入磁碟之前**就退出，所以使用者看到"
            "彈窗且匯出產生 **0 個 CSV** —— 即使第 545 行的 "
            "`dlgSaveAs.Show` 已經觸發並捕獲了檔名。\n\n"
            "靜態 schema 互相印證（基於 "
            "`analysis/dump/tables.json`）：`DYNASTIES` **有** "
            "`c_dynasty` 與 `c_dynasty_chn`；`BIOG_MAIN` **有** "
            "`c_female`（也在 "
            "`tests/test_schema.py::REQUIRED_COLUMNS` 之中）。"
            "所以這 **不是** source-side 欄位 rename / removal —— "
            "欄位在源表都在，SELECT 只是沒投影。純粹是 "
            "recordset projection mismatch。\n\n"
            "**與 Issue #23（LookAtAssociations × CmdNeo4j "
            "target-column mismatch）不同。** Issue #23 是 JET "
            "3061（SQL parser）：INSERT 目標欄位列表引用了不存在"
            "的目標表欄位。本 issue 是 JET 3265（DAO 欄位集合查"
            "找）：VBA 層的 `Recordset!field` 引用在執行期欄位"
            "集合中找不到對應欄位。相同的表面根因類別（缺欄位"
            "引用）但**不同的觸發面**（DAO 欄位查找 vs SQL "
            "parser）和**不同的修法面**（SELECT 投影 vs INSERT "
            "目標列表）。不應併入 Issue #23。\n\n"
            "**與 Issue #21（LookAtGroupData × CmdNeo4j 未保護"
            "的 `.MoveFirst`）也不同。** Issue #21 是 DAO 3021"
            "「No current record」—— 對空但有效的 recordset 的"
            "狀態機錯誤。本 issue 是對非空但欄位集合中不存在某"
            "欄位的 recordset 做欄位存取的錯誤。不同 DAO 錯誤碼、"
            "不同修法路徑。"
        ),
        "steps_en": [
            "Open **LookAtPlace**.",
            "Pick a substantive **address** in the address picker "
            "(any single c_addr_id with substantial associated "
            "biography data; the matrix `_make_place_fixtures` "
            "first fixture, `place_addr_<top_addr_by_indexed_persons>`, "
            "gives ZZ_SCRATCH_PLACE_PEOPLE ≈ 5,764 rows on the "
            "current dump and is the same fixture the existing "
            "Place × CmdGIS / CmdPajek tests use).",
            "On the form, set the tab to **Places** (TabPlaces=0; "
            "the current cross-form CmdNeo4j test handles this "
            "in `_seed_query_inputs`).  Tick **ChkIndividual** "
            "and leave **ChkOffice / ChkAssoc / ChkPosting / "
            "ChkEntry** unchecked.",
            "Click **Run Query** — CmdQuery completes cleanly; "
            "scratch tables are populated (ZZ_SCRATCH_PEOPLE, "
            "ZZ_SCRATCH_PLACE_PEOPLE, ZZ_SCRATCH_PLACE_AGG all "
            "non-empty).",
            "Click **Neo4j** (the export button).",
            "A **Run-time error 3265 — Item not found in this "
            "collection.** popup appears (or in the headless / "
            "driver-instrumented run, the equivalent ZZ_TEST_DEBUG "
            "marker `LookAtPlace:ERR Item not found in this "
            "collection.` is written).  The Neo4j export "
            "produces **0 CSV files** — no `People_*.csv`, no "
            "`Places_*.csv`, nothing.\n\n"
            "Verified end-to-end via the probe at "
            "`analysis/probe_place_cmdneo4j.py` (PR #120, merged "
            "commit `8f94276`); chain-order-first failing "
            "reference identified by the static investigation at "
            "`analysis/investigate_place_cmdneo4j_item_not_found"
            ".{py,md}` (PR #121, merged commit `97e1162`).",
        ],
        "steps_zh": [
            "打開 **LookAtPlace**。",
            "在地址挑選器選一個有實質資料的 **address**（任何"
            "關聯了大量傳記資料的 c_addr_id；matrix "
            "`_make_place_fixtures` 第一條 fixture "
            "`place_addr_<top_addr_by_indexed_persons>` 在當前 "
            "dump 上能讓 ZZ_SCRATCH_PLACE_PEOPLE ≈ 5,764 行 —— "
            "與現有 Place × CmdGIS / CmdPajek 測試共用同一條 "
            "fixture）。",
            "在表單上將 tab 設為 **Places**（TabPlaces=0；當前 "
            "cross-form CmdNeo4j 測試在 `_seed_query_inputs` 中"
            "已處理）。勾 **ChkIndividual**，不勾 **ChkOffice / "
            "ChkAssoc / ChkPosting / ChkEntry**。",
            "點 **Run Query** —— CmdQuery 順利完成；scratch 表"
            "都被填充（ZZ_SCRATCH_PEOPLE、"
            "ZZ_SCRATCH_PLACE_PEOPLE、ZZ_SCRATCH_PLACE_AGG 都"
            "非空）。",
            "點 **Neo4j**（匯出按鈕）。",
            "彈出 **執行時錯誤 3265 —— Item not found in this "
            "collection.** 對話框（或 headless ／ "
            "driver-instrumented 跑法下，對應的 "
            "`LookAtPlace:ERR Item not found in this "
            "collection.` ZZ_TEST_DEBUG marker 被寫入）。Neo4j "
            "匯出產出 **0 份 CSV** —— 沒有 `People_*.csv`、"
            "沒有 `Places_*.csv`，什麼都沒有。\n\n"
            "已通過 probe `analysis/probe_place_cmdneo4j.py` "
            "端到端驗證（PR #120，已 merge `8f94276`）；具體"
            "的失敗引用點由靜態調查 "
            "`analysis/investigate_place_cmdneo4j_item_not_found"
            ".{py,md}` 確認（PR #121，已 merge `97e1162`）。",
        ],
        "fix_en": (
            "**Recommended upstream CBDB fix:** extend the SELECT "
            "projection in `Form_LookAtPlace.vb:643-647` to "
            "include the three columns the loop reads:\n\n"
            "```vb\n"
            ...
            "    \"ON ZZ_SCRATCH_P_TEXT.c_person_id = BIOG_MAIN.c_personid\"\n"
            "```\n\n"
            "The FROM / JOIN structure already brings the source "
            "tables into scope; this is purely a missing-from-"
            "SELECT issue.  Three columns added; nothing else "
            "changes.\n\n"
            "**Driver-side workaround option** (separate brief; "
            "NOT part of this issue): mirror the "
            "`_PER_FORM_CMDGIS_PATCHES` Issue #4 (`GISFrame -> "
            "CodeFrame`), Issue #5 (`ChkIDs -> False`), and "
            "Issue #23 (`c_index_addr_type_code -> c_addr_type` "
            "INSERT target rewrite) workaround patterns with a "
            "per-form rewrite that extends the SELECT projection "
            "literal inside `Form_LookAtPlace`'s CmdNeo4j_Click "
            "only.  This would unblock the test suite without "
            "requiring a CBDB-side fix."
        ),
        "fix_zh": (
            "**推薦的上游 CBDB 修復：** 擴展 "
            "`Form_LookAtPlace.vb:643-647` 的 SELECT 投影，"
            "包含迴圈讀取的 3 個欄位：\n\n"
            "```vb\n"
            "tQueryStr = \"SELECT DISTINCT \" & _\n"
            "    \"ZZ_SCRATCH_P_TEXT.c_person_id, \" & _\n"
            ...
            "    \"ON ZZ_SCRATCH_P_TEXT.c_person_id = BIOG_MAIN.c_personid\"\n"
            "```\n\n"
            "FROM / JOIN 結構已經把源表帶進範圍；這純粹是 "
            "SELECT 缺欄位的問題。增加 3 個欄位，其他不變。\n\n"
            "**Driver-side workaround 選項**（另一份 brief；"
            "本 issue 不包含）：仿照 `_PER_FORM_CMDGIS_PATCHES` "
            "裡 Issue #4（`GISFrame -> CodeFrame`）、Issue #5"
            "（`ChkIDs -> False`）和 Issue #23（"
            "`c_index_addr_type_code -> c_addr_type` INSERT "
            "目標重寫）的寫法，在僅限於 `Form_LookAtPlace` 的 "
            "CmdNeo4j_Click 內做一個擴展 SELECT 投影字面量的 "
            "per-form rewrite。這能讓測試套不依賴上游修復就跑"
            "通。"
        ),
        "screenshots": [],
        "severity_en": (
            "P1 — Visible crash on a normal user click (any "
            "LookAtPlace CmdNeo4j export with a non-empty "
            "place-people result; the JET 3265 fires "
            "deterministically at line 757's `!c_dynasty` read "
            "on the matrix Place fixture per PR #120's runtime "
            "probe and the static investigation per PR #121)"
        ),
        "severity_zh": (
            "P1 — 正常用戶點擊下的可見報錯（只要 LookAtPlace "
            "查詢有非空的 place-people 結果，CmdNeo4j 匯出就會"
            "觸發；JET 3265 在第 757 行 `!c_dynasty` 讀取上"
            "穩定復現，runtime 證據在 PR #120 的 probe，靜態"
            "調查在 PR #121）"
        ),
    },
]
