...
On Error GoTo Err_CmdImportTextCategories_Click
    ...
    If Not tQuit Then
        ...
        tStrSQL = "INSERT INTO InputErrorList ( c_ID ) SELECT TempImportList.ImportID " + _
            "FROM TEXT_BIBLCAT_CODES RIGHT JOIN TempImportList ON TEXT_BIBLCAT_CODES.c_text_cat_code = TempImportList.ImportID " + _
        ...
        '  copy the good IDs
        '
        tStrSQL = "INSERT INTO ZZ_TEXT_BIBLCAT_CODES ( c_text_cat_code ) SELECT DISTINCT TempImportList.ImportID " + _
            "FROM TEXT_BIBLCAT_CODES INNER JOIN TempImportList ON TEXT_BIBLCAT_CODES.c_text_cat_code = TempImportList.ImportID"
...
On Error GoTo Err_CmdNeo4j_Click
    ...
    '
    Dim tRstPeople As DAO.Recordset, tRstTexts As DAO.Recordset, tRstTextCodes As DAO.Recordset, tRstPlace As DAO.Recordset
    ...
    If dlgSaveAs.Show = -1 Then
        ...
        '
        tQueryStr = "SELECT DISTINCT ZZ_SCRATCH_P_TEXT.c_addr_id, ZZ_SCRATCH_P_TEXT.c_addr_name, ZZ_SCRATCH_P_TEXT.c_addr_chn, " + _
    ...
    If dlgSaveAs.Show = -1 Then
        ...
        gStream.Open
            '
        tQueryStr = "SELECT DISTINCT ZZ_SCRATCH_P_TEXT.c_person_id, BIOG_MAIN.c_index_addr_id, " + _
    ...
    tQueryStr = "SELECT DISTINCT BIOG_MAIN.c_index_addr_type_code, BIOG_ADDR_CODES.c_addr_desc AS c_index_addr_type_desc, " + _
            ...
            "WHERE ( ((BIOG_MAIN.c_index_addr_type_code) > 0) AND ((BIOG_MAIN.c_index_addr_id) > 0) )"
    ...
    If dlgSaveAs.Show = -1 Then
        ...
        Else
            tStr = "TextID" + tC + "TextPY" + tC + "TextHZ" + tC + "TextCategoryCode" + tC + "TextCategoryDesc" + tC + "TextCategoryHZ"
        End If
        gStream.WriteText tStr, adWriteLine
        '
        ' get the codes
        ...
        tQueryStr = "SELECT DISTINCT ZZ_SCRATCH_BIOG_TEXT_DATA.c_textid, ZZ_SCRATCH_BIOG_TEXT_DATA.c_title, " + _
                        ...
                        "ZZ_SCRATCH_BIOG_TEXT_DATA.c_text_cat_desc, ZZ_SCRATCH_BIOG_TEXT_DATA.c_text_cat_desc_chn " + _
    ...
    If dlgSaveAs.Show = -1 Then
        ...
        gStream.Open
            '
        tQueryStr = "SELECT DISTINCT ZZ_SCRATCH_BIOG_TEXT_DATA.c_role_id, ZZ_SCRATCH_BIOG_TEXT_DATA.c_role_desc, " + _
...
Private Sub CmdQuery_Click()
    ...
    tQueryInsertStr = "INSERT INTO ZZ_SCRATCH_BIOG_TEXT_DATA (c_personid, c_name, c_name_chn, c_index_year, c_index_year_type_code, c_dy, " + _
            "c_sex, c_addr_id, c_textid, c_title, c_title_chn, c_role_id, c_role_desc, c_role_desc_chn, c_source ) "

    tQuerySelectStr = "SELECT BIOG_MAIN.c_personid, BIOG_MAIN.c_name, BIOG_MAIN.c_name_chn, BIOG_MAIN.c_index_year, BIOG_MAIN.c_index_year_type_code, " + _
        "BIOG_MAIN.c_dy, IIf(BIOG_MAIN.c_female, 'F', 'M') AS c_sex, BIOG_MAIN.c_index_addr_id, BIOG_TEXT_DATA.c_textid, TEXT_CODES.c_title, " + _
        "TEXT_CODES.c_title_chn, BIOG_TEXT_DATA.c_role_id, TEXT_ROLE_CODES.c_role_desc, TEXT_ROLE_CODES.c_role_desc_chn, BIOG_TEXT_DATA.c_source "
    
    If tUseDynasties And gFromDynasty > -2 Then
        If tUseAddr Then
            tQueryFromStr = "FROM TEXT_ROLE_CODES INNER JOIN ( ( DYNASTIES INNER JOIN ( BIOG_MAIN INNER JOIN ZZ_SCRATCH_ADDR ON BIOG_MAIN.c_index_addr_id = ZZ_SCRATCH_ADDR.c_addr_id ) " + _
            "ON DYNASTIES.c_dy = BIOG_MAIN.c_dy ) INNER JOIN ( ( TEXT_CODES INNER JOIN ZZ_TEXT_BIBLCAT_CODES ON TEXT_CODES.c_bibl_cat_code = ZZ_TEXT_BIBLCAT_CODES.c_text_cat_code ) " + _
            "INNER JOIN BIOG_TEXT_DATA ON TEXT_CODES.c_textid = BIOG_TEXT_DATA.c_textid ) ON BIOG_MAIN.c_personid = BIOG_TEXT_DATA.c_personid ) ON TEXT_ROLE_CODES.c_role_id = BIOG_TEXT_DATA.c_role_id "
        Else
            tQueryFromStr = "FROM TEXT_ROLE_CODES INNER JOIN ( ( TEXT_CODES INNER JOIN ZZ_TEXT_BIBLCAT_CODES ON TEXT_CODES.c_bibl_cat_code = ZZ_TEXT_BIBLCAT_CODES.c_text_cat_code ) " + _
            "INNER JOIN ( ( DYNASTIES INNER JOIN BIOG_MAIN ON DYNASTIES.c_dy = BIOG_MAIN.c_dy ) INNER JOIN BIOG_TEXT_DATA ON BIOG_MAIN.c_personid = BIOG_TEXT_DATA.c_personid ) " + _
            "ON TEXT_CODES.c_textid = BIOG_TEXT_DATA.c_textid ) ON TEXT_ROLE_CODES.c_role_id = BIOG_TEXT_DATA.c_role_id "
        End If
    Else
        If tUseAddr Then
            tQueryFromStr = "FROM ZZ_SCRATCH_ADDR INNER JOIN ( TEXT_ROLE_CODES INNER JOIN ( ( TEXT_CODES INNER JOIN ZZ_TEXT_BIBLCAT_CODES ON TEXT_CODES.c_bibl_cat_code = ZZ_TEXT_BIBLCAT_CODES.c_text_cat_code ) " + _
            "INNER JOIN ( BIOG_MAIN INNER JOIN BIOG_TEXT_DATA ON BIOG_MAIN.c_personid = BIOG_TEXT_DATA.c_personid ) ON TEXT_CODES.c_textid = BIOG_TEXT_DATA.c_textid ) " + _
            "ON TEXT_ROLE_CODES.c_role_id = BIOG_TEXT_DATA.c_role_id ) ON ZZ_SCRATCH_ADDR.c_addr_id = BIOG_MAIN.c_index_addr_id "
        Else
        tQueryFromStr = "FROM TEXT_ROLE_CODES INNER JOIN ( ( TEXT_CODES INNER JOIN ZZ_TEXT_BIBLCAT_CODES ON TEXT_CODES.c_bibl_cat_code = ZZ_TEXT_BIBLCAT_CODES.c_text_cat_code ) " + _
            "INNER JOIN ( BIOG_MAIN INNER JOIN BIOG_TEXT_DATA ON BIOG_MAIN.c_personid = BIOG_TEXT_DATA.c_personid ) ON TEXT_CODES.c_textid = BIOG_TEXT_DATA.c_textid ) " + _
    ...
    cmdSQL.Execute tRecCount
    '
    '  the next step is to make the table of people from the Text records
    '
    If tRecCount > 0 Then
        '
    ' add the additional information
    '
    tQueryStr = "UPDATE ( ( (ZZ_SCRATCH_BIOG_TEXT_DATA LEFT JOIN INDEXYEAR_TYPE_CODES " + _
            "ON ZZ_SCRATCH_BIOG_TEXT_DATA.c_index_year_type_code = INDEXYEAR_TYPE_CODES.c_index_year_type_code) LEFT JOIN ADDR_CODES " + _
            "ON ZZ_SCRATCH_BIOG_TEXT_DATA.c_addr_id = ADDR_CODES.c_addr_id ) LEFT JOIN TEXT_CODES ON ZZ_SCRATCH_BIOG_TEXT_DATA.c_source = TEXT_CODES.c_textid) " + _
            "LEFT JOIN DYNASTIES ON ZZ_SCRATCH_BIOG_TEXT_DATA.c_dy = DYNASTIES.c_dy " + _
            "SET ZZ_SCRATCH_BIOG_TEXT_DATA.c_dynasty = [DYNASTIES].[c_dynasty], ZZ_SCRATCH_BIOG_TEXT_DATA.c_dynasty_chn = [DYNASTIES].[c_dynasty_chn], " + _
                "ZZ_SCRATCH_BIOG_TEXT_DATA.c_index_year_type_desc = [INDEXYEAR_TYPE_CODES].[c_index_year_type_desc], " + _
                "ZZ_SCRATCH_BIOG_TEXT_DATA.c_index_year_type_hz = [INDEXYEAR_TYPE_CODES].[c_index_year_type_hz], " + _
                "ZZ_SCRATCH_BIOG_TEXT_DATA.c_addr_name = [ADDR_CODES].[c_name], ZZ_SCRATCH_BIOG_TEXT_DATA.c_addr_chn = [ADDR_CODES].[c_name_chn], " + _
                "ZZ_SCRATCH_BIOG_TEXT_DATA.x_coord = [ADDR_CODES].[x_coord], ZZ_SCRATCH_BIOG_TEXT_DATA.y_coord = [ADDR_CODES].[y_coord], " + _
    ...
    tQueryStr = "UPDATE ( ( ZZ_SCRATCH_BIOG_TEXT_DATA INNER JOIN ( TEXT_BIBLCAT_CODES INNER JOIN TEXT_CODES ON TEXT_BIBLCAT_CODES.c_text_cat_code = TEXT_CODES.c_bibl_cat_code ) " + _
                "ON ZZ_SCRATCH_BIOG_TEXT_DATA.c_textid = TEXT_CODES.c_textid ) INNER JOIN TEXT_BIBLCAT_CODE_TYPE_REL ON TEXT_BIBLCAT_CODES.c_text_cat_code = TEXT_BIBLCAT_CODE_TYPE_REL.c_text_cat_code ) " + _
                "INNER JOIN TEXT_BIBLCAT_TYPES ON TEXT_BIBLCAT_CODE_TYPE_REL.c_text_cat_type_id = TEXT_BIBLCAT_TYPES.c_text_cat_type_id " + _
            "SET ZZ_SCRATCH_BIOG_TEXT_DATA.c_text_cat_code = [TEXT_CODES].[c_bibl_cat_code], ZZ_SCRATCH_BIOG_TEXT_DATA.c_text_cat_desc = [TEXT_BIBLCAT_CODES].[c_text_cat_desc], " + _
                "ZZ_SCRATCH_BIOG_TEXT_DATA.c_text_cat_desc_chn = [TEXT_BIBLCAT_CODES].[c_text_cat_desc_chn], ZZ_SCRATCH_BIOG_TEXT_DATA.c_text_cat_pinyin = [TEXT_BIBLCAT_CODES].[c_text_cat_pinyin], " + _
                "ZZ_SCRATCH_BIOG_TEXT_DATA.c_text_cat_type_id = [TEXT_BIBLCAT_CODE_TYPE_REL].[c_text_cat_type_id], ZZ_SCRATCH_BIOG_TEXT_DATA.c_text_cat_type_desc = [TEXT_BIBLCAT_TYPES].[c_text_cat_type_desc], " + _
...
On Error GoTo Err_CmdSaveTextCategories_Click
    ...
    If dlgSaveAs.Show = -1 Then
        ...
        ' process the table
        '
        tStr = "SELECT ZZ_TEXT_BIBLCAT_CODES.c_text_cat_code, TEXT_BIBLCAT_CODES.c_text_cat_desc, TEXT_BIBLCAT_CODES.c_text_cat_desc_chn " + _
...
Private Sub CmdStoreID_Click()
    ...
    tStrQuery = "INSERT INTO ZZ_STORE_PERSON_ID ( c_personid ) SELECT DISTINCT ZZ_SCRATCH_P_TEXT.c_person_id FROM ZZ_SCRATCH_P_TEXT"
