...
- **recommendation:** UI direct simulation is mechanically impossible for these buttons on the LookAtStatus form: the form's design has no UI controls named CmdPajek / CmdGephi (verified by recursive COM Controls enumeration AND UIA descendants scan). The Cmd<X>_Click subs exist as VBA-only code with no UI binding. Pywinauto cannot click what doesn't exist on the form. Recommend pure maintainer-line; the maintainer scope must include adding the missing UI buttons alongside the cleanup-rebind fix, OR documenting these subs as orphaned/legacy.
...
- [x] Probe-quality note: strategy 1 (`uia_auto_id_lookup`) raised `TypeError("IUIA.build_condition() got an unexpected keyword argument 'auto_id'")` because pywinauto 0.6.9's `descendants()` accepts `auto_id` as a keyword filter for child_window / find_elements but NOT for `descendants()` directly. **This does NOT change the verdict**: even if strategy 1 had run cleanly, it would have returned an empty result because there is no UIA Button matching that auto_id (verified independently by strategy 2's `uia_name_lookup` which DID run cleanly and returned 0 matches). Strategy 1's failure is at the pywinauto API surface, not at the underlying "the button doesn't exist" reality.
- [x] Confirmed surprising-but-orthogonal finding: the form IS visible in UIA — but under its **Caption** `'Look at Status 查詢社會區分'`, NOT its `Name` `'LookAtStatus'`. So the verdict is NOT "form not visible to pywinauto" — it is specifically "the buttons don't exist on the form", which holds for both COM and UIA enumeration.
- [x] Next step that should NOT be autopiloted: any maintainer scope expansion (e.g. requesting that the missing UI buttons be added) is a separately-scoped follow-up; this PR records the diagnostic finding but does not unilaterally expand the maintainer-line.
- [x] **New finding worth recording**: `Form_LookAtStatus.vb` has `CmdPajek_Click`, `CmdGephi_Click`, `CmdUCINet_Click` Sub bodies but the form's UI design has no controls named CmdPajek / CmdGephi / CmdUCINet (verified by recursive COM enumeration AND grep of the .vb dump for any `Me.Cmd<X>.*` references — zero matches). Compare to `Form_LookAtAssociations.vb` which has both the Sub bodies AND the matching `CmdPajek.Enabled` / `CmdGephi.Enabled` / `Me.CmdPajek.Caption` lines indicating the buttons exist. The Status form's export Subs appear to be orphaned VBA-only code with no UI binding. Whether this becomes a separate canonical issue is for the planner to decide.
