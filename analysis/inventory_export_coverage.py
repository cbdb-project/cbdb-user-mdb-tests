...
EXPORT_TEST_MANIFEST: list[dict] = [
    ...
    {"form": "LookAtKinship", "button": "CmdUCINet",
     ...
     "notes": (
         ...
         "test-side workaround; no driver change in that "
         "PR.  KNOWN FIXTURE-FRAGILE per Issue #22's "
         "Kinship sibling-form confirmation (probe "
         "investigate/kinship-cmducinet-sibling-risk "
         "commit 154bb4b): this test passes only because "
         "the matrix-supplied person 3211's kin network "
         "happens to contain no Han-character c_name "
         "values; switching to a fixture whose network "
         "reaches a Han-name person reproduces the same "
         "VBA error 5 documented in canonical Issue #22.  "
         "Coverage status remains 'covered' because the "
         "test does pass on the current matrix fixture, "
         "but maintainers should treat this cell as "
         "Issue-#22-vulnerable for future fixture / dump "
         "changes."
     )},
