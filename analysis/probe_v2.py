"""Probe with LinkListInit pre-patched to avoid relink hang."""
...
# Pre-patch LinkListInit so NAVIGATION_PANE.Form_Open skips relink
conn = pyodbc.connect(
    "DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};"
    f"DBQ={WORK};", autocommit=True
)
cur = conn.cursor()
cur.execute("SELECT c_path, c_dataset FROM LinkListInit")
row = cur.fetchone()
log(f"LinkListInit before: c_path={row.c_path!r}  c_dataset={row.c_dataset!r}")
# Set c_path to OUR working copy path so the equality check passes
cur.execute(f"UPDATE LinkListInit SET c_path = '{str(WORK).replace(chr(39), chr(39)*2)}'")
cur.execute("SELECT c_path FROM LinkListInit")
log(f"LinkListInit after:  c_path={cur.fetchone().c_path!r}")
conn.close()
...
try:
    ...
    log("AutomationSecurity = 1")
except Exception as e: log(f"AutoSec err: {e}")
app.Visible = True
log("OpenCurrentDatabase...")
app.OpenCurrentDatabase(str(WORK))
log("OpenCurrentDatabase returned")
...
paths = [
    ...
    ("Forms!X!CmdQuery.SetFocus + SendKeys ENTER",
     lambda: (
         app.Forms("LookAtEntry").Controls("CmdQuery").SetFocus(),
         app.SendKeys(" ", True),  # space activates focused button
     )),
    ("OnClick set + SetFocus + SendKeys",
     lambda: (
         setattr(app.Forms("LookAtEntry").Controls("CmdQuery"), "OnClick",
                 "[Event Procedure]"),
         app.Forms("LookAtEntry").Controls("CmdQuery").SetFocus(),
         app.SendKeys(" ", True),
     )),
    ("RunCommand acCmdEnter (=11)",
     lambda: (
         app.Forms("LookAtEntry").Controls("CmdQuery").SetFocus(),
         app.RunCommand(11),
     )),
    ("DoCmd.SetWarnings False + SendKeys",
     lambda: (
...
for label, fn in paths:
    ...
    try:
        ...
        log(f"    FAIL ({type(e).__name__}): {e!s} ; rows={count()}")
