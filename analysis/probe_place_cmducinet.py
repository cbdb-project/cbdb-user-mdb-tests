...
PICKER_RATIONALE = (
    ...
    "(3 links) + CJK name; chosen as a minimal-load picker after "
    "earlier probe v2 on Kaifeng addr 100658 ran CmdQuery for "
    ">240s and was followed by COM unavailability on the next "
    "set_form_tag call (the link from heavier CmdQuery work to "
    "the COM failure is suggestive but NOT proven — see iteration "
...
def _classify(result: dict) -> dict:
    ...
    if outcome == "file_appeared_no_err" and sections_match_static \
        ...
        rationale = (
            "**Place CmdUCINet runtime behaviour remains "
            "unobserved.** Every probe iteration on this dump "
            "ended with the second `set_form_tag` call (for "
            "CmdUCINet) raising "
            "`com_error('The RPC server is unavailable.')`, "
            "before any CmdUCINet code path could execute.\n\n"
            "Honest framing of the failure: this is "
            "**post-form-work COM bridge instability in the "
            "current Place probe shape**.  The narrower claim "
            "'CmdQuery on the Place form is the trigger' is "
            "NOT supported by the evidence: iteration 3 "
            "deliberately bypassed CmdQuery (synthetic-row "
            "injection via pyodbc) and the next "
            "`set_form_tag` STILL failed RPC-unavailable "
            "after a separately-failed direct-COM Requery "
            "attempt.  So whether CmdQuery is necessary to "
            "trigger the instability is unresolved; what we "
            "CAN say is that under the current probe shape, "
            "the COM bridge becomes unavailable after the "
            "form-side work that precedes the CmdUCINet fire "
            "(CmdQuery in iters 1/2/4, a failed Requery in "
            "iter 3).\n\n"
            "**Bucket label caveat: `still_needs_better_"
            "fixture` is an imperfect fit.**  The brief's "
            "fixed bucket vocabulary forces this label, but "
            "the actual blocker is NOT fixture insufficiency "
            "— it is unresolved probe-shape / COM "
            "instability.  Picker size, checkbox state, and "
            "synthetic-row injection were all tried; none of "
            "them is the root cause.  Reading this bucket as "
            "'try a different fixture and Place will be "
            "characterized' would be a misread.\n\n"
            "What this PR's evidence DOES support:\n"
            "  - Place CmdUCINet runtime still unobserved.\n"
            "  - The current probe shape hits unresolved "
            "COM/RPC instability after form-side work.\n"
            "  - The static prediction (ADO Stream + UTF-8, "
            "3-section file shape, FSO path commented out) "
            "remains the only available characterization of "
            "Place CmdUCINet's writer.\n\n"
            "What a follow-up would need (out of scope for "
            "this PR's brief boundary): either (a) a driver-"
            "side change targeted at the post-form-work "
            "instability, after a separate investigation "
            "localizes which form-side operation actually "
            "triggers it, OR (b) a different probe shape "
            "that fires CmdUCINet via a path that doesn't "
            "go through `click_via_timer` (e.g. pywinauto "
            "UI click on the Access window — but "
...
def _write_md(result: dict, classification: dict) -> None:
    ...
    md.append("This probe was run in four iterations before the "
              ...
              "all of them showed the SAME terminal failure "
              "(`com_error('The RPC server is unavailable.')` "
              "on the second `set_form_tag` call, the one that "
              "fires CmdUCINet). The form-side work that "
              "preceded that failure differed across "
              "iterations — CmdQuery in iters 1/2/4, a "
              "failed direct-COM Requery in iter 3 — so "
              "CmdQuery is *not* a proven common cause; "
              "see closing paragraph for the narrowest claim "
              "the evidence supports. Iterations recorded so "
              "future probe authors don't re-run the same "
              "dead ends:")
    md.append("")
    ...
    md.append("3. **Synthetic-row injection bypassing CmdQuery** "
              "(open form, INSERT 4 Kinship rows directly via "
              "pyodbc, then attempt subform Requery via direct "
              "COM). CmdQuery was NOT fired in this iteration. "
              "The subform Requery via `sess.app.Forms("
              "'LookAtPlace').Controls('frmZZZ_PLACE')."
              "Form.Requery()` raised "
              "`AttributeError('Access.Application.Forms')`; "
              "the next set_form_tag still failed with RPC "
              "unavailable. NB: this iteration shows that "
              "CmdQuery is NOT a necessary precondition for "
              "the post-form-work COM failure — a failed "
              "direct-COM Requery is also enough to leave the "
              "bridge unavailable.")
    ...
    md.append("Common factor: every iteration that reached the "
              "second `set_form_tag` call (for CmdUCINet) "
              "failed the same way. The instability is NOT "
              "picker-size, NOT checkbox-state, NOT row-count "
              "dependent. The narrowest claim the evidence "
              "supports is **post-form-work COM bridge "
              "instability in the current Place probe shape** "
              "— the COM bridge becomes unavailable after the "
              "form-side work that precedes the CmdUCINet "
              "fire (CmdQuery in iters 1/2/4, a failed Requery "
              "in iter 3). Whether CmdQuery specifically is "
              "necessary to trigger the instability is "
              "unresolved.")
    md.append("")
