...
**Oracle classification (D0).** Every test is classified by its oracle
in `docs/oracle-classification.json` (generated by
`analysis/build_oracle_registry.py`, pinned by
`tests/test_oracle_classification.py`): **A** = real VBA × independent
oracle (the only class that counts as VBA verification); **B** = real
VBA × `cbdb_replay` oracle (`test_vba_differential.py`,
`test_vba_matrix.py` — divergence-only, a shared bug passes); **C** =
replay × golden (`test_lookatentry.py`, `test_other_lookat_forms.py`,
`test_exports.py` — tests the Python, NOT the .mdb); **NA** = infra/unit.
